...
function imprimirSemBotoes() {
    ...
    const botoes = document.querySelectorAll(
        '.excluir-btn, .custom-btn, .remove-btn, #inserir-btn, .imprimir'
    );
    ...
    // Aguarda um pouco para garantir que os botões foram ocultados
    setTimeout(() => {
        window.print();

        // Após a impressão, exibe os botões novamente
        botoes.forEach(botao => {
            botao.style.display = '';
        });
    }, 100);
